...
def parse_consensus(consensus_proto, resource):
    ...
    try:
        ...
        consensus = resource.get('header', {}).get('consensus', None)
        LOGGER.error(
            'The validator sent a resource with %s %s',
            'a missing consensus' if consensus is None else 'an invalid consensus:',
            consensus or '')
        raise errors.ResourceConsensusInvalid()
